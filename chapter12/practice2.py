...
mySock.send(CMD)
all_data = b'' #Created a byte string for append all bytes received 
while True:
    data :bytes = mySock.recv(1024) # Increased the buffer size to 1024 bytes 
    if len(data) < 1 :
        break
    all_data += data  # Appending all chunks of data received to one byte object
    # mySock.close() #Closes socket but not needed as the while loop handles it when all data is received
    
text = all_data.decode()
print(text)
